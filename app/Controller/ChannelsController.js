...
function sendPushNotification (replierId, notifierId) {
    ...
    User.findOne({ _id: replierId })
        ...
        .exec(function(err, user) {
            if(user) {
                var message = user.public_details.first_name + " just sent you a new Chat!"
...
function getHistory(channelId, skip, limit, fn) {
    ...
    Channel.checkNInsert(param, function(err, channel) {
        if(err) {
            ...
            if(channel) {
                ...
                //looped message inserts
                Message.insert(msg1Params, function(err, messageFirst) {
                    if(err) {
                        res.status(500).json(err);
                    } else {
                        //push in Channels
                        Channel.findByIdAndUpdate(channelId, {
                            $push: {
                                messages: messageFirst._id
                            },
                            last_message: messageFirst._id
                                                    ...
                                                    if(err) {
                                                        ...
                                                        Channel.findByIdAndUpdate(channelId, {
                                                            ...
                                                            if(err) {
                                                                ...
                                                                sendPushNotification(replierId, param._contactUserId);
                                                                ...
                                                                getHistory(channelId, skip, limit, function(err, history) {
                                                                    if(err) {
                                                                        res.status(500).json(err);
                                                                    } else {
                                                                        if(history) {
                    ...
                    if(channels) {
                        ...
                        async.each(channels, function(channel, callback){
                            ...
                            User.findOne({_id: _contactUserId})
                                .lean()
                                .select('email social_details public_details preferences photos posts created')
                                .populate({
                                    path: 'photos',
                                ...
                                .populate({
                                    path: 'posts',
                                    match: { status: true },
                                    select: 'post_message created responses venue'
                                })
                                .exec(function(err, user) {
                                    if(err) {
                                        res.status(500).json(err);
